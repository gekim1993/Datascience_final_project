...
combined_chain_fastfood = 
  ...
  right_join(combined_chain)
combined_chain_fastfood_gradea = 
  gradea_neighborhood %>%
  select(neighborhood, grade_percent) %>%
  right_join(combined_chain_fastfood)

combined_model =  
  left_join(combined_chain_fastfood_gradea, health_neighborhood, by = "neighborhood")

lm(obesity ~ fastfood_percent + racewhite_rate + age0to44_rate + poverty + smoking + exercise, combined_model) %>%
  broom::tidy() %>%
  kable()

plot(obesity ~ fastfood_percent, combined_model)
plot(obesity ~ chain_percentage, combined_model)


lm(obesity ~ chain_percentage + racewhite_rate + age0to44_rate + poverty + smoking + exercise, combined_model) %>%
  summary() 
