...
The combined dataset "neighborhood_chain" has `r nrow(neighborhood_chain)` observations. Also, there were 33 different chain restaurants extracted. 
...
We plot neighborhoods in NYC against their percentage of chain restaurants and group them by borough. We can see that all the neighborhoods with percentage of chain restuarants less than 5% are all in Brooklyn except Greenwhich village and Soho in Manhattan. Neighborhoods in Queens and Manhattan are spread out across low to high percentage of chain restaurants while most of the neighborhoods in Bronx and Staten Island have high percentages. The neighborhood with the highest percentage of chain restaurants are Throgs neck and Co-op City in Bronx with 16.5% of chain restaurats out of all restaurats.
...
We have three final models each having a health outcome (prevalence of obesity, prevalence of diabetes, and stroke hospitalization rate) as the dependent variable and percentage of fastfood as the main predictor. Obesity and percentage of fastfood restuarants model gives the most significant results with p-value of the main predictor being 0.0005364 (<0.01). In other words, at significance level of 1%, every 10% increase in the number of fastfood restaurants (0.1 unit increase in the percentage of fastfood restaurants) in a neighborhood is associated with 5.94% increase in the neighborhood's obesity prevalence, while adjusting for other factors.

The p-value of the regression coefficients for model with outcomes as diabetes/stroke are both around 0.003 (<0.01), indicating there is a significant linear relationship between diabetese/stroke and percentage of fastfood restaurants at 1% significance level. To put in other words, at significance level of 1%, every 10% increase in the number of fastfood restaurants in a neighborhood is associated with 1.75% increase in the neighborhood's diabetes prevalence, while adjusting for other factors. Furthermore, every 10% increase in the number of fastfood restaurants in a neighborhood is associated with increase in 57.35 stroke hospitalizations per 100,000 adults in the neighborhood, while adjusting for other factors.
